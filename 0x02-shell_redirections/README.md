...
_krbfast:*:246:-2:Kerberos FAST Account:/var/empty:/usr/bin/false
```
Tips: "Think of it as *cat*, what is at the end of it?"
#### Repo:
* GitHub repository: **alx-system_engineering-devops**
* Directory: **0x02-shell_redirections**
* File: **4-lastlines**
...
### 11. Don't just count you directories, make your directories count
...
Script that displays lines containing pattern "root" from the file **/etc/passwd**
```
$ ./14-findthatword
root:*:0:0:System Administrator:/var/root:/bin/sh
daemon:*:1:1:System Services:/var/root:/usr/bin/false
...
Script displays all line of the file **/etc/ssh/sshd_config** starting with a letter.
...
Script with a command that finds all empty files and directories in the current directory and all sub-directories.
...
* The files should be sorted by byte values, but case-insensitive (file **aaa** should be listed before file **bbb**, file .**b** should be listed before file **a**, and file **Rona** should be listed after the file **jay**)
* One file name per line
* The listing should end with a new line
...
### 25. Acrostic
An acrostic is a poem (or other form of writing) in which the first letter (or syllable, or word) of each line (or paragraph, or\
other recurring feature in the text) spells out a word, message or the alphabet. The word comes from the French acrostiche\
from post-classical Latin acrostichis). As a form of constrained writing, an acrostic can be used as a mnemonic device to\
aid memory retrieval. [Read more](https://en.wikipedia.org/wiki/Acrostic).
